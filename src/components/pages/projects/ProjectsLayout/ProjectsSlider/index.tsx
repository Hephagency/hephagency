...
export default function ProjectsSlider({projects}: ProjectsSliderProps){
    ...
    return (
       <div className="xl:max-w-7/12 xl:grow xl:basis-0 xl:relative">
            <Swiper
            slidesPerView={1.25}
            ref={swiperRef}
            effect="creative"
            ...
            <div className="flex justify-end mt-6 px-4 md:px-6 xl:px-7.5 gap-2.5 xl:absolute xl:top-full xl:right-0">
                ...
                side="right"
                buttonClassName={clsx(
                    (currentIndex === projects.length - 1 || (swiperRef.current && (swiperRef.current.swiper.isEnd || swiperRef.current.swiper.isLocked))) && "opacity-50 pointer-events-none"
                )}
                />
            </div>
       </div>
